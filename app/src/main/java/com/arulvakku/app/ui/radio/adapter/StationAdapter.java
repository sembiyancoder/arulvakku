...
public class StationAdapter extends RecyclerView.Adapter<StationAdapter.StationHolder> {
    ...
    protected class StationHolder extends SwappingHolder implements View.OnClickListener, View.OnLongClickListener, MaterialFavoriteButton.OnFavoriteChangeListener {
        ...
        public void bindStation(Station station) {
            ...
            mNameTextView.setText("அருள்வாக்கு வானொலி");
            mUrlTextView.setText(mStation.url);

            // show either playback animation, buffering icon or stop icon
            boolean isSelected = mStation.equals(MyApplication.sDatabase.selectedStation);
            boolean isBuffering = MyApplication.sDatabase.bufferingState == BufferEvent.BUFFERING;
            boolean isPaused = MyApplication.sDatabase.playbackState == PlaybackEvent.PAUSE;
            boolean isStopped = MyApplication.sDatabase.playbackState == PlaybackEvent.STOP;
            boolean isFavorite = MyApplication.sDatabase.getStations().contains(mStation); // TODO HashSet?
